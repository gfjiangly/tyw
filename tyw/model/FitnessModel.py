...
class FitnessModel(object):
    ...
    """
    def __init__(self, persen_info):
        self.persen_info = persen_info
        if 'min_heart_rate' in persen_info:
            self.min_heart_rate = persen_info['min_heart_rate']
        else:
            self.min_heart_rate = cfg.TEST.FITNESS_MODEL.MIN_HEART_RATE
        self.max_heart_rate = persen_info['max_heart_rate']
